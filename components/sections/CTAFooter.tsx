  ...
  return (
    <>
      ...
      <footer className="bg-ink py-20 md:py-24">
        <div className="container-n7 grid gap-12 lg:grid-cols-[260px_1fr] lg:gap-16">
          <div className="text-[150px] font-semibold leading-none md:text-[190px] lg:self-start">
            ...
            <div className="grid gap-10 md:grid-cols-3 md:gap-12">
              {footerLocations.map((location) => (
                <address key={location.city + location.address} className="not-italic">
                  <h3 className="text-[17px] font-medium leading-none tracking-[-0.01em] text-white">
                    {location.city}
                  </h3>
                  <p className="mt-6 max-w-[310px] text-[16px] leading-[1.38] tracking-[-0.01em] text-white/60">
                    {location.address}
                  </p>
                </address>
              ))}
            ...
            <div className="grid gap-10 md:grid-cols-3 md:gap-12">
              {footerLinkGroups.map((group) => (
                <nav key={group.title} aria-label={group.title}>
                  <h3 className="mb-5 text-[17px] font-medium leading-none tracking-[-0.01em] text-white">
                    {group.title}
                  </h3>
                  <ul className="space-y-3.5">
                    {group.links.map((link) => (
                      <li key={link}>
                        <a
                          href="#"
                          className="group grid max-w-[215px] grid-cols-[minmax(0,1fr)_18px] items-start gap-4 text-[16px] leading-[1.25] tracking-[-0.01em] text-white/60 transition hover:text-white"
                        >
                          <span>{link}</span>
                          <ArrowRight className="mt-0.5 text-cyan transition group-hover:translate-x-1" size={17} strokeWidth={1.8} />
                        </a>
                      </li>
                    ))}
                  </ul>
